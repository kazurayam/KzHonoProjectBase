...
このコードは `@hono/zod-openapi` と link://https://github.com/honojs/middleware/tree/main/packages/swagger-ui[`@hono/swagger-ui`] をimportしている。これを成功させるには `myAPIserver` プロジェクトの外部依存性を解決する必要がある。
...
[source,json]
----
include::../myAPIserver/package.json[]
----
...
[source,json]
----
include::../myWEBserver/package.json[]
----
...
link:https://zenn.dev/umaidashi/articles/d1097ef5c9af50["hono(bun)をcloudflare workersにGitHub actions ci/cdで自動デプロイ"]
...
link:https://github.com/kazurayam/KzHonoProjectBase/[KzHonoProjectBase] レポジトリで Settings > Secrets and variables > Actions のメニューをたどり、New repository secret のボタンを押した。
...
[source,yaml]
----
include::../myWEBserver/.github/workflows/deploy.yml[]
----
